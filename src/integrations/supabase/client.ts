...
export async function fetchProjectFiles(projectId: string): Promise<ProjectFile[]> {
  try {
    ...
    // Transform the storage files into our application's ProjectFile type
    // Using a simple transformation to avoid type recursion
...
export async function uploadProjectFile(
  ...
  try {
    ...
    // Create our ProjectFile object without any complex type operations
    const result: ProjectFile = {
      id: `file-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
      projectId,
      filename: file.name,
